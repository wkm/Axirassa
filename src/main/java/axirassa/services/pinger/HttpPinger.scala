...
class HttpPinger(client : InstrumentedHttpClient = new InstrumentedHttpClient) {
  ...
  var triggers = new HashMap[Class[_], Trigger]
  ...
  def ping(entity : PingerEntity) = {
    ...
    try {
      ...
      statistic.setLatency(client.latency)
      statistic.setResponseTime(client.responseTime)
      statistic.setStatusCode(response.getStatusLine.getStatusCode())
      statistic.setResponseSize(client.responseContent.length())
      statistic.setUncompressedSize(0)

      printf("Latency: %dms Response: %dms Status: [%d] URL: %s\n", statistic.getLatency,
